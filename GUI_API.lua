--[[
    ASSLUA GUI API v1.8
    - Kompletter Neuaufbau der Layout- und Skalierungslogik für maximale Stabilität.
    - Basiert auf dem ursprünglichen, einfachen Layout-Ansatz.
]]
...
-- ## GUI.new - Zurück zum einfachen und stabilen Layout ##
function GUI.new(title)
    ...
    -- MainFrame: Zurück zum originalen, einfachen (0,0) AnchorPoint
    ...
    self:_setupDynamicScaling() -- Ruft die NEUE Skalierungsfunktion auf
...
-- ## NEUE, STABILE SKALIERUNGSFUNKTION ##
function GUI:_setupDynamicScaling()
    -- Diese Funktion ist jetzt viel einfacher und expliziter.
    -- Sie skaliert NUR die Hauptcontainer und lässt deren Position unberührt.
    self.MainFrame:GetPropertyChangedSignal("Size"):Connect(function()
        -- Die Höhe der DragBar bleibt fest
        local dragBarHeight = self.DragBar.AbsoluteSize.Y

        -- Passe die Größe des TabContainers an
        -- Die Breite bleibt fest, die Höhe füllt den Rest aus.
        self.TabContainer.Size = UDim2.new(0, 150, 1, -dragBarHeight)

        -- Passe die Größe des ContentContainers an
        -- Die Breite UND Höhe füllen den Rest aus.
        self.ContentContainer.Size = UDim2.new(1, -150, 1, -dragBarHeight)
    end)
end

-- Die restlichen Funktionen bleiben wie in der letzten funktionierenden Version
function GUI:CreateTab(name)
    ...
    -- Hier können alle anderen Add... Funktionen (AddToggle etc.) stehen
    return tabApi
end
